...
func TestBuildDNSRecord_NilWhenNoAnnotation(t *testing.T) {
	...
	if rec != nil {
		t.Fatalf("expected nil without dns-name annotation, got %+v", rec)
	}
}

func TestBuildDNSRecord_ProducesCorrectSpec(t *testing.T) {
	svc := mkSvc(func(s *corev1.Service) {
		s.Annotations = map[string]string{AnnotationDNSName: "api.example.com"}
	})
	rec := BuildDNSRecord(svc, []string{"203.0.113.12", "203.0.113.10", "203.0.113.11"})
	if rec == nil {
		t.Fatal("expected non-nil DNSRecord")
	}
...
func TestBuildDNSRecord_SortsTargets(t *testing.T) {
	svc := mkSvc(func(s *corev1.Service) {
		s.Annotations = map[string]string{AnnotationDNSName: "svc.example.com"}
	})
	rec := BuildDNSRecord(svc, []string{"10.0.0.3", "10.0.0.1", "10.0.0.2"})
...
func TestBuildDNSRecord_EmptyTargets(t *testing.T) {
	svc := mkSvc(func(s *corev1.Service) {
		s.Annotations = map[string]string{AnnotationDNSName: "svc.example.com"}
	})
	rec := BuildDNSRecord(svc, nil)
	if rec == nil {
		t.Fatal("expected non-nil DNSRecord even with empty targets")
	}
	if len(rec.Spec.Targets) != 0 {
		t.Fatalf("expected empty targets, got %v", rec.Spec.Targets)
	}
}

func TestBuildDNSRecord_DoesNotMutateInput(t *testing.T) {
	svc := mkSvc(func(s *corev1.Service) {
		s.Annotations = map[string]string{AnnotationDNSName: "svc.example.com"}
	})
	ips := []string{"10.0.0.3", "10.0.0.1"}
	original := append([]string(nil), ips...)
	_ = BuildDNSRecord(svc, ips)
	if !sameStringSet(ips, original) {
		t.Fatalf("BuildDNSRecord mutated the input slice: got %v want %v", ips, original)
	}
}
